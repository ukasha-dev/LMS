...
class Multi_common_model extends MY_Model
{
    ...
    */
    public function getStudentCount($school_array = [])
    {
        ...
        // Process default database (main branch)
        ...
        // Get gender counts using subqueries
    ...
    ");
        ...
        $school['total_student'] = isset($row->total_student) ? (int)$row->total_student : 0;
        $school['male_count'] = isset($row->male_count) ? (int)$row->male_count : 0;
        $school['female_count'] = isset($row->female_count) ? (int)$row->female_count : 0;
        $school['db_name'] = $default_db;
        $school['session'] = $current_db->session;

        $results[$default_db] = $school;

        // Process branch databases
        ...
        if (!empty($branches)) {
            foreach ($branches as $branch_key => $branch_value) {
                ...
                if (!isset($school_array[$db_dynamic_name])) {
                    continue;
                }
                ...
                // Get gender counts for branch
                $db_dynamic->select("
                COUNT(*) as total_student,
                SUM(CASE WHEN LOWER(students.gender) = 'male' THEN 1 ELSE 0 END) as male_count,
                SUM(CASE WHEN LOWER(students.gender) = 'female' THEN 1 ELSE 0 END) as female_count
            ");
                $db_dynamic->from('students');
